...
function showCategory(category) {
    let productArr = products[category]
    let htmlString = ""
    productArr.forEach(function (product) {
        htmlString = htmlString + getProductMarkup(product)
    })

    productEl.insertAdjacentHTML("afterbegin",htmlString)
}
...
function getProductMarkup(product) {
    let htmlString = `<div className="product">`
    htmlString = htmlString + `<div>${product.name}</div>`
    htmlString = htmlString + `<img src="${product.imageUrl}" alt="">`
    htmlString = htmlString + `<div>${product.price}</div>`
    htmlString = htmlString + `<a href="https://example.com/producs/${product.id}">Подробнее</a>`
    htmlString = htmlString + `</div>`
    return htmlString
}

let buttElements = document.querySelectorAll("button")
let productEl =  document.querySelector(".products")

buttElements.forEach(function (button){
